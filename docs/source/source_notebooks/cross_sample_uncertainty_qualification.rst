...
replicates previously produced identical pointwise tables. The checkpoint
...
colored by this metric. The dashed ellipse marks the high-distance region
(top 5%); it is not a confidence region.
...
The primary table is ``pointwise_transformation_variability.csv``. The spatial
map and distribution use ``dist_mean`` and are saved as PDF, PNG and editable
SVG; ``uncertainty_report.json`` records the mean-distance summary.
