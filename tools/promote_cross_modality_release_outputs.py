#!/usr/bin/env python3
"""Copy repeated ST-to-H&E and ATAC-to-ST results into public docs.

The computational notebooks remain the executable workflows. This script
renders their repeated-run results into the static filenames used by the
tutorial and records the observed agreement between runs.
"""
...
def promote_atlas(args: argparse.Namespace, static_dir: Path) -> dict:
    ...
    if not cell_order_exact or not pair_table_repeat_exact:
        raise AssertionError("Atlas discrete repeat check failed")
...
def promote_he(args: argparse.Namespace, static_dir: Path) -> dict:
    ...
    # this check verifies that the current files are present and records the
...
def main() -> None:
    ...
    destination = (
        ...
        "cross_modality_repeat_summary.json"
    )
